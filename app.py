...
def render_table(df, key="default"):
    ...
    disp["Zvezdice"]  = disp["Zvezdice"].apply(stars_html)
...
def _render_segment_content(df, seg_key, t_label):
...
if not search_btn:
    ...
    st.info("Vnesi termine, izberi objekte, nato klikni **Poišči cene**.")
    st.stop()
...
for t_idx, (t_in, t_out) in enumerate(termini):
    def _make_progress(idx=t_idx):
        def _cb(pct, msg):
            overall = (idx + pct) / n_termini * 0.85
            prog.progress(min(overall, 0.85), text=msg)
        return _cb
    batch = apify_fetch_all(dict(urls_per_adults), t_in, t_out, token, _make_progress())
...
for seg_key in selected_segments:
    ...
    for (t_in, t_out), t_label in zip(termini, t_labels):
        ...
        rows  = assemble_segment(seg_key, sheets_data[seg_key], t_in, t_out, adults, batch)
...
<div class="segment-header">
  ...
  &nbsp;&nbsp;<span>{seg['description']}</span>
        ...
        if len(termini) == 1:
            _render_segment_content(all_data[seg_key][t_labels[0]], seg_key, t_labels[0])
        else:
            t_tabs = st.tabs(t_labels)
            for t_tab, t_label in zip(t_tabs, t_labels):
                with t_tab:
                    _render_segment_content(all_data[seg_key][t_label], seg_key, t_label)
